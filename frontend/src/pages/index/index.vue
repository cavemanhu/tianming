    ...
    <view class="new-features">
      ...
      <view class="feature-grid">
        <view 
          ...
          @tap="goToFeature(feature.value)"
        >
          <view class="feature-icon" :style="{ background: feature.color + '20' }">{{ feature.icon }}</view>
          <view class="feature-info">
            <text class="feature-name" :style="{ color: feature.color }">{{ feature.label }}</text>
            <text class="feature-desc">{{ feature.desc }}</text>
          </view>
          <view class="feature-arrow" :style="{ color: feature.color }">→</view>
        </view>
      </view>
    </view>
...
export default {
  ...
  data() {
    return {
      fortuneTypes: [
        { value: 'year', label: '年运', icon: '📅', desc: '全年运势分析', color: '#FF8C42' },
        { value: 'month', label: '月运', icon: '🌙', desc: '本月运势预测', color: '#9B59B6' },
        { value: 'love', label: '姻缘', icon: '💕', desc: '情感姻缘分析', color: '#FF6B9D' },
        { value: 'career', label: '事业', icon: '💼', desc: '职场发展指导', color: '#4ECDC4' }
      ],
      
      // 新增功能入口
      newFeatures: [
        { value: 'fatename', label: '智能取名', icon: '✍️', desc: '八字五行·诗经典故', color: '#FFD700' },
        { value: 'yijing', label: '易经占卜', icon: '☯️', desc: '64卦·铜钱摇卦', color: '#D4AF37' }
      ]
    }
  },
  ...
  methods: {
    ...
    goToFeature(feature) {
      if (feature === 'fatename') {
        uni.navigateTo({ url: '/pages/fatename/index' })
      } else if (feature === 'yijing') {
        uni.navigateTo({ url: '/pages/yijing/index' })
      }
    },
